...
/// E2E UI tests for hakuke terminal.
/// Tests real keyboard/mouse interactions via XCUITest.
/// Requires hakuke installed at /Applications/Hakuke.app.
///
/// Pattern from Ghostty: terminate + relaunch for clean state before each test.
final class TerminalUITests: HakukeUITestCase {
    ...
    override class func setUp() {
        ...
        let app = XCUIApplication(bundleIdentifier: "com.hakuke.terminal")
        app.terminate()
        sleep(1)
        app.launch()
        sleep(2)
    ...
    override func setUp() {
        ...
        app = XCUIApplication(bundleIdentifier: "com.hakuke.terminal")
    ...
    func testAppLaunches() {
        XCTAssertTrue(app.exists, "hakuke should be running")
    }

    func testAppIsAccessory() {
        // hakuke is LSUIElement — no Dock icon, no main window
    ...
    func testCloseLastTab_reopensNew() {
        // Close the only tab — hakuke should auto-create a new one
        app.typeKey("w", modifierFlags: .command)
        sleep(1)
