...
struct GhosttyThemeCatalogTests {
    ...
    /// A theme that sets every color Hakuke reads, in the format Ghostty ships.
    ...
    private func withThemeFile(_ contents: String, _ body: (String) throws -> Void) throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("hakuke-theme-test-\(UUID().uuidString)")
    ...
    @Test func missingFile_returnsNil() {
        #expect(Self.ghosttyReady)
        let path = NSTemporaryDirectory() + "/hakuke-does-not-exist-\(UUID().uuidString)"
