...
/// already drifted once: the shipped bundle was `com.hakuke.app` while `project.yml`
/// and the Help text both told users to reset `com.hakuke.terminal`, a domain that
/// never existed. Nothing at runtime notices that, so pin it here.
@Suite(.serialized)
struct AppIdentityTests {

    private static var repoRoot: URL {
        URL(fileURLWithPath: #filePath)      // …/Hakuke/Tests/HakukeTests/<this file>
            .deletingLastPathComponent()      // HakukeTests
            .deletingLastPathComponent()      // Tests
            .deletingLastPathComponent()      // Hakuke
            .deletingLastPathComponent()      // repo root
    }

    private static func infoPlist() throws -> [String: Any] {
        let url = repoRoot.appendingPathComponent("Hakuke/Resources/Info.plist")
