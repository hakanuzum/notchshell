...
///
/// Run with: NOTCHSHELL_TEST_GHOSTTY=1 swift test --filter GhosttyConfigProbe
@MainActor
@Suite(.serialized)
struct GhosttyConfigProbeTests {
    ...
    @Test func probeConfigGet() throws {
        ...
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("notchshell-probe-\(UUID().uuidString)")
    ...
    @Test func probeConfigFileLayering() throws {
        ...
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("notchshell-layer-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    @Test func probeDualThemeResolution() throws {
        ...
        func background(of themeValue: String) -> String {
            let dir = URL(fileURLWithPath: NSTemporaryDirectory())
                .appendingPathComponent("notchshell-dual-\(UUID().uuidString)")
            try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
            defer { try? FileManager.default.removeItem(at: dir) }
